...
namespace Sourav.Tools.SearchTool.Editor
{
    ...
    {
        private static IEnumerable<Type> _unityTypes;
        ...
        {
            if (_unityTypes==null)
                _unityTypes = AppDomain.CurrentDomain.GetAssemblies()
        ...
        private static IEnumerable<SearchItemInfo> RefreshInfos(string itemName, Func<string[], IEnumerable<Type>, IEnumerable<SearchItemInfo>> findAction, bool returnAllOnEmptySearch = false)
        {
            ...
            if (string.IsNullOrEmpty(itemName))
            {
                return returnAllOnEmptySearch ? findAction(new string[] {}, _unityTypes).Take(MAX_ITEMS_COUNT) : NoResults;
            }

            var searchTerms = itemName.Split(new[] {' '}, StringSplitOptions.RemoveEmptyEntries).ToList();
            var results = ResultsFor(searchTerms, _unityTypes, findAction);
